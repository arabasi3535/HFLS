  ...
  return (
    <div style={{ marginBottom: 12 }}>
      <div style={{ fontSize: 11, fontWeight: 600, color: "#64748b", marginBottom: 4, display: "flex", alignItems: "center", gap: 5 }}>
        {label}
        {required && <span style={{ fontSize: 10, background: "#dc2626", color: "#fff", borderRadius: 3, padding: "1px 5px", fontWeight: 700 }}>필수</span>}
        {optional && <span style={{ fontSize: 10, background: "#64748b", color: "#fff", borderRadius: 3, padding: "1px 5px", fontWeight: 700 }}>선택</span>}
      </div>
      {children}
    </div>
  );
...
function ReqCard({ r, catColor }) {
  return (
    <div style={{ background: "#f8fafc", borderRadius: 8, padding: "10px 13px", marginBottom: 6, borderLeft: "3px solid " + catColor }}>
      ...
      {r.warn && <div style={{ fontSize: 11, color: "#b91c1c", background: "#fef2f2", borderRadius: 4, padding: "4px 8px", marginTop: 5 }}>⚠️ {r.warn}</div>}
    </div>
  );
}
...
export default function App() {
  ...
  const analyze = () => {
    ...
    const curReqs = getRequirements(form.buildingType, area, gf, bf, occ, "era3");
    let ext = null;
    if (form.hasExt && form.extDate) {
      ...
      if (extEra) {
        ...
        ext = { era: extEra, reqs: getRequirements(form.buildingType, ea, ef, bf, occ, extEra.id), area: ea, floors: ef, firewall: form.extFirewall };
      }
    }
    ...
    <div style={{ fontFamily: "'Noto Sans KR', sans-serif", background: "#f0f4f8", minHeight: "100vh" }}>
      ...
      <div style={{ background: "linear-gradient(135deg,#1e3a5f 0%,#1d4ed8 100%)", color: "#fff", padding: "18px 24px" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ fontSize: 11, opacity: .7, marginBottom: 3 }}>🔥 HFLS v2.0 — Historical Fire Law Support System</div>
          <div style={{ fontSize: 22, fontWeight: 700, letterSpacing: "-0.3px" }}>소방시설 연혁 법령 적용 지원 시스템</div>
          <div style={{ fontSize: 12, opacity: .8, marginTop: 4 }}>
            건축허가일 기준 소방법령 자동 판별 · 의무 소방시설 조회 (별표1·2) · 증축 특례 이중기준 처리 (시행령 제15조)
          </div>
        </div>
      </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "minmax(280px,360px) 1fr", gap: 16, alignItems: "start" }}>
          ...
          <div style={{ background: "#fff", borderRadius: 14, padding: 20, boxShadow: "0 2px 8px rgba(0,0,0,.08)", position: "sticky", top: 16 }}>
            ...
            <div style={{ fontSize: 11, color: "#94a3b8", marginBottom: 14, paddingBottom: 10, borderBottom: "1px solid #f1f5f9" }}>
              <span style={{ background: "#fef2f2", border: "1px solid #fca5a5", borderRadius: 3, padding: "1px 6px", color: "#dc2626", fontWeight: 700, marginRight: 4 }}>필수</span> 항목을 모두 입력 후 분석 버튼을 눌러주세요
            </div>

            <Field label="건축허가일 (건축허가서 기준)" required>
            ...
            <Field label="특정소방대상물 용도 [별표2 제5조]" required>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              <div>
                <div style={{ fontSize: 11, fontWeight: 600, color: "#64748b", marginBottom: 4, display: "flex", alignItems: "center", gap: 4 }}>
                  연면적(㎡) <span style={{ fontSize: 10, background: "#dc2626", color: "#fff", borderRadius: 3, padding: "1px 5px" }}>필수</span>
                </div>
                <input type="number" value={form.totalArea} placeholder="예: 1500"
                  onChange={e => set("totalArea", e.target.value)}
                  style={{ ...inputStyle, borderColor: errors.totalArea ? "#ef4444" : "#e2e8f0" }} />
                {errors.totalArea && <div style={{ fontSize: 11, color: "#ef4444", marginTop: 2 }}>{errors.totalArea}</div>}
              </div>
              <div>
                <div style={{ fontSize: 11, fontWeight: 600, color: "#64748b", marginBottom: 4, display: "flex", alignItems: "center", gap: 4 }}>
                  지상 층수 <span style={{ fontSize: 10, background: "#dc2626", color: "#fff", borderRadius: 3, padding: "1px 5px" }}>필수</span>
                </div>
                <input type="number" value={form.groundFloors} placeholder="예: 5"
                  onChange={e => set("groundFloors", e.target.value)}
                  style={{ ...inputStyle, borderColor: errors.groundFloors ? "#ef4444" : "#e2e8f0" }} />
                {errors.groundFloors && <div style={{ fontSize: 11, color: "#ef4444", marginTop: 2 }}>{errors.groundFloors}</div>}
              </div>
              <div>
                <div style={{ fontSize: 11, fontWeight: 600, color: "#64748b", marginBottom: 4, display: "flex", alignItems: "center", gap: 4 }}>
                  지하 층수 <span style={{ fontSize: 10, background: "#64748b", color: "#fff", borderRadius: 3, padding: "1px 5px" }}>선택</span>
                </div>
                <input type="number" value={form.basementFloors} placeholder="없으면 0"
                  onChange={e => set("basementFloors", e.target.value)}
                  style={{ ...inputStyle }} />
              </div>
              <div>
                <div style={{ fontSize: 11, fontWeight: 600, color: "#64748b", marginBottom: 4, display: "flex", alignItems: "center", gap: 4 }}>
                  수용인원(명) <span style={{ fontSize: 10, background: "#64748b", color: "#fff", borderRadius: 3, padding: "1px 5px" }}>선택</span>
                </div>
                <input type="number" value={form.occupants} placeholder="예: 100"
            ...
            <div style={{ background: "#f8fafc", borderRadius: 10, padding: 12, border: "1px solid #e2e8f0", margin: "14px 0" }}>
              <label style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, fontWeight: 700, cursor: "pointer", color: "#374151" }}>
                ...
                증축 이력 있음 (증축 특례 적용)
                <span style={{ fontSize: 10, background: "#64748b", color: "#fff", borderRadius: 3, padding: "1px 5px" }}>선택</span>
              </label>
              {form.hasExt && (
                <div style={{ marginTop: 10 }}>
                  {/* 시행령 제15조 단서조항 안내 */}
                  <div style={{ fontSize: 11, color: "#7c3aed", lineHeight: 1.8, marginBottom: 10, background: "#f5f3ff", borderRadius: 6, padding: "8px 10px", border: "1px solid #c4b5fd" }}>
                    <b>📌 시행령 제15조(증축 시 소방시설 설치) 적용 원칙</b><br />
                    <span style={{ color: "#1d4ed8" }}>원칙:</span> 기존 부분 + 증축 부분 모두 <b>증축 허가일 기준</b> 적용<br />
                    <span style={{ color: "#16a34a" }}>단서(예외):</span> 기존 부분에 <b>기존 소방시설 기준</b> 적용 가능 조건 →<br />
                    <span style={{ paddingLeft: 12, display: "block", color: "#b45309" }}>
                      ① 기존 부분과 증축 부분이 <b>내화구조(耐火構造)로 된 바닥·벽</b>과<br />
                      &nbsp;&nbsp;&nbsp;<b>방화문(자동방화셔터 포함)</b>으로 구획(區劃)된 경우<br />
                      ② 자동차 생산공장 등 화재위험이 낮은 대상 (화재안전기준 지정)<br />
                      ③ 국가유산청장이 지정한 국가유산 건축물 (화재안전기준 지정)
                    </span>
                    <span style={{ color: "#dc2626", fontWeight: 700 }}>⚠️ 방화구획 미형성 시 → 기존 부분도 증축 허가일 기준 소방시설 전면 적용</span>
                  </div>

                  {/* 방화구획 여부 체크 */}
                  <label style={{ display: "flex", alignItems: "flex-start", gap: 8, fontSize: 12, cursor: "pointer", color: "#374151", background: "#fffbeb", borderRadius: 6, padding: "8px 10px", border: "1px solid #fde68a", marginBottom: 10 }}>
                    <input type="checkbox" checked={form.extFirewall} onChange={e => set("extFirewall", e.target.checked)}
                      style={{ width: 14, height: 14, accentColor: "#d97706", marginTop: 1, flexShrink: 0 }} />
                    <span><b>기존 부분과 증축 부분이 내화구조 바닥·벽 및 방화문(자동방화셔터 포함)으로 구획됨</b><br />
                      <span style={{ color: "#92400e" }}>(체크 시: 기존 부분 → 건축허가일 기준 / 증축 부분 → 증축 허가일 기준 이중 적용)</span></span>
                  </label>

                  {[["증축 허가일", "extDate", "date", null, true],
                    ["증축 연면적(㎡)", "extArea", "number", "예: 300", false],
                    ["증축 층수", "extFloors", "number", "예: 2", false]].map(([lb, k, t, ph, req]) => (
                    <div key={k} style={{ marginBottom: 8 }}>
                      <div style={{ fontSize: 11, fontWeight: 600, color: "#64748b", marginBottom: 3, display: "flex", alignItems: "center", gap: 4 }}>
                        {lb}
                        {req
                          ? <span style={{ fontSize: 10, background: "#dc2626", color: "#fff", borderRadius: 3, padding: "1px 5px" }}>필수</span>
                          : <span style={{ fontSize: 10, background: "#64748b", color: "#fff", borderRadius: 3, padding: "1px 5px" }}>선택</span>}
                      </div>
          ...
          <div id="result-panel">
            {!result ? (
              <div style={{ background: "#fff", borderRadius: 14, padding: 60, textAlign: "center", boxShadow: "0 2px 8px rgba(0,0,0,.08)" }}>
                ...
                <div style={{ fontSize: 13, color: "#94a3b8", marginBottom: 28 }}>건축허가일 기준 적용 법령 및 의무 소방시설이 자동 조회됩니다</div>
              ...
              <>
                ...
                <div style={{ background: "#fff", borderRadius: 14, padding: 18, boxShadow: "0 2px 8px rgba(0,0,0,.08)", marginBottom: 12 }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(200px,1fr))", gap: 8 }}>
                    {[["화재안전기준", result.era.standard + " (" + result.era.standardAuth + ")"],
                      ...
                      ["화재안전조사 근거", result.era.safetyInspection]].map(([k, v]) => (
                  ...
                  <div style={{ marginTop: 10, fontSize: 12, color: "#64748b", background: "#f1f5f9", borderRadius: 7, padding: "7px 11px" }}>
                    ℹ️ {result.era.note}
                  </div>
                </div>
                ...
                <div style={{ background: "#fff", borderRadius: 14, boxShadow: "0 2px 8px rgba(0,0,0,.08)" }}>
                  ...
                  <div style={{ padding: 18 }}>
                    ...
                    {tab === 0 && (
                      <div>
                        ...
                        <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 14 }}>
                          {CATS.map(cat => (
                            <div key={cat} style={{ fontSize: 10, background: CAT_COLORS[cat] + "15", border: "1px solid " + CAT_COLORS[cat] + "44", borderRadius: 5, padding: "3px 8px", color: CAT_COLORS[cat], fontWeight: 700 }}>
                              {CAT_ICONS[cat]} {cat}
                            </div>
                          ))}
                        </div>
                        ...
                        <div style={{ fontSize: 11, color: "#94a3b8", background: "#fafafa", borderRadius: 8, padding: "8px 12px", marginTop: 8, lineHeight: 1.6 }}>
                          ※ 본 결과는 참고용입니다. 정확한 적용 여부는 건축허가 당시 시행령·NFSC/NFTC 원문 및 부칙 경과규정을 반드시 확인하십시오.
                        </div>
                      </div>
                    )}
                      ...
                      return (
                        <div>
                          <div style={{ fontSize: 12, color: "#64748b", marginBottom: 14, padding: "8px 12px", background: "#f8fafc", borderRadius: 8 }}>
                            <b>{result.era.label}</b> 기준 대비 <b>현행(2022.12.1~)</b> 기준의 추가·변경 사항
                          </div>
                          {added.length > 0 && <>
                    ...
                    {tab === 2 && result.ext && (
                      <div>
                        ...
                        <div style={{ background: "#fffbeb", border: "1px solid #fde68a", borderRadius: 8, padding: "12px 14px", marginBottom: 14, fontSize: 12, lineHeight: 1.9 }}>
                          <b>📌 증축 특례 근거: 소방시설 설치 및 관리에 관한 법률 시행령 제15조 제1항</b><br />
                          <span style={{ color: "#374151" }}>
                            원칙: 기존·증축 부분 모두 <b>증축 허가일 당시 기준</b> 적용<br />
                            단서: 기존 부분과 증축 부분이 <b>내화구조로 된 바닥·벽</b>과 <b>방화문(자동방화셔터 포함)</b>으로 구획된 경우
                            → 기존 부분에 <b>건축허가일({result.form.approvalDate}) 당시 기준</b> 적용 가능
                          </span>
                          {result.ext.firewall
                            ? <div style={{ marginTop: 6, padding: "5px 10px", background: "#f0fdf4", borderRadius: 6, color: "#166534", fontWeight: 700, border: "1px solid #bbf7d0" }}>
                                ✅ 방화구획 형성 확인 → 이중기준 적용 (기존: 건축허가일 기준 / 증축: 증축 허가일 기준)
                              </div>
                            : <div style={{ marginTop: 6, padding: "5px 10px", background: "#fef2f2", borderRadius: 6, color: "#b91c1c", fontWeight: 700, border: "1px solid #fecaca" }}>
                                ⚠️ 방화구획 미확인 → 기존 부분도 증축 허가일({result.form.extDate}) 기준 전면 적용 원칙
                              </div>
                          }
                        </div>
                          ...
                          {[
                            ...
                            <div key={label}>
                              ...
                              {reqs.length === 0
                                ...
                                : reqs.map((r, i) => (
                                  <div key={i} style={{ fontSize: 11, background: bg, borderRadius: 7, padding: "7px 10px", marginBottom: 4 }}>
                    ...
                    {tab === tabs.length - 1 && (
                      <div>
                        <div style={{ fontSize: 12, color: "#64748b", marginBottom: 14 }}>소방 법령 체계 변천 전체 이력 (1958 ~ 현재)</div>
                        {LAW_ERAS.map(era => (
                          <div key={era.id} style={{
                            ...
                            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 6 }}>
                              ...
                              {era.id === result.era.id && (
                                <span style={{ fontSize: 11, background: era.color, color: "#fff", borderRadius: 5, padding: "3px 10px" }}>📍 해당 건물 적용 시대</span>
                              )}
                            </div>
                            <div style={{ fontSize: 11, color: "#64748b", marginTop: 3 }}>{era.start} ~ {era.end === "2099-12-31" ? "현재" : era.end}</div>
                            <div style={{ fontSize: 13, fontWeight: 600, color: "#1e293b", marginTop: 5 }}>{era.lawName}</div>
                            <div style={{ fontSize: 11, color: "#64748b", marginTop: 2 }}>화재안전기준: {era.standard} ({era.standardAuth})</div>
                        ...
                        <div style={{ background: "#fef9c3", border: "1px solid #fde047", borderRadius: 10, padding: "12px 15px", fontSize: 12, color: "#854d0e", lineHeight: 1.8 }}>
                          ...
                          기존 건축물은 원칙적으로 건축허가 당시 기준 적용. 단, 개정 법령 부칙에서 소급 적용을 명시한 경우 현행 기준 적용 의무 발생.
                          자체점검 및 화재안전조사 시 각 개정의 부칙 경과규정을 반드시 확인하십시오.
                        </div>
                      </div>
                    )}
      ...
      <div style={{ background: "#1e293b", color: "#94a3b8", textAlign: "center", padding: "16px 20px", fontSize: 11, lineHeight: 1.8 }}>
        HFLS v2.0 — 소방시설 연혁 법령 적용 지원 시스템 &nbsp;|&nbsp; 참고용 시스템으로 법적 효력이 없습니다<br />
        특정소방대상물 용도: 소방시설법 시행령 별표2 (2025.11.25) &nbsp;|&nbsp; 소방시설 분류: 별표1 (5개 설비군) &nbsp;|&nbsp; 증축특례: 시행령 제15조<br />
