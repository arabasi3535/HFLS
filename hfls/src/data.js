...
[별표 1] 소방시설(제3조 관련)(소방시설 설치 및 관리에 관한 법률 시행령).pdf
pdf

HFLS 시스템을 수정하고 싶어, 현재 특정소방대상물이 별표 2의 기준에서 17,19,20,21,24,25,26의 분류가 누락되어 있어 한번  첨부자료 찾아보고 수정하고 복합건축물은 가와 나로 분류되는데 이것도 가와 나로 분류해주고  소방시설의 종류도 별표1자료를 기준으로 표기하며, 사용승인일이 아니고 건축허가일로 변경 부탁해, 그리고 증축특례표기에 기존부분에 기존소방시설을 넣으려면 방화구획이 되어 있어야해 그 단서조항을 소방시설설치법 시행령 15조 증축특례기준을 보고 입력해줘  별표2의 특정소방대상물 용도구분이 가장 중요한 부분이닌깐 그 부분을 확실히 검토해서 정확하게 입력해주고   그리고 필수입력사항에는 필수라는 말을 넣고 선택입력사항에는 선택이라는 문구를 넣어줘

더 보기
오전 9:33

건축법령 기준 시스템 전면 재구성 및 정확화 계획 수립
건축법령 기준 시스템 전면 재구성 및 정확화 계획 수립
이제 두 파일을 모두 업데이트하겠습니다. 별표1·2, 시행령 제15조 단서조항을 반영합니다.


명령 8개 실행함, 2개의 파일 생성됨
명령 8개 실행함, 2개의 파일 생성됨
모든 내용이 올바르게 반영되었습니다. 이제 파일을 출력합니다.


명령 실행함, 파일 읽음
명령 실행함, 파일 읽음
두 파일 모두 수정 완료했습니다. 주요 변경사항을 정리합니다.

HFLS v2.0 수정 내역
data.js — 특정소방대상물 분류 완성 (별표2)
신규 추가 7개 용도 (별표2 기준)

조항	ID	내용
제17호	hazmat	위험물 저장 및 처리 시설 (제조소등·가스시설)
제19호	animal_plant	동물 및 식물 관련 시설 (축사·도축장·온실·재배사 등)
제20호	recycling	자원순환 관련 시설 (하수처리·고물상·폐기물 재활용·처분)
제21호	correctional	교정 및 군사시설 (교도소·구치소·치료감호·소년원·군사시설)
제24호	cemetery	묘지 관련 시설 (화장시설·봉안당·동물화장·건조장)
제25호	tourism	관광 휴게시설 (야외음악당·야외극장·어린이회관·관망탑·휴게소)
제26호	funeral	장례시설 (장례식장·동물전용 장례식장)
복합건축물 분리 (제30호)

mixed_a — 가목: 둘 이상 용도 복합 (비주거)
mixed_b — 나목: 근생·판매·업무·숙박·위락 + 주택 복합
소방시설 종류 — 별표1 제1호~제5호 완전 반영

소화용수설비 (제4호) 신설: 상수도소화용수설비·소화수조·저수조
App.js — UI 수정
사용승인일 → 건축허가일 전면 변경 (헤더·라벨·결과·푸터 모두)

필수/선택 배지 추가

🔴 필수: 건축허가일, 특정소방대상물 용도, 연면적, 지상 층수, 증축허가일
⚫ 선택: 지하 층수, 수용인원, 증축 관련 항목
증축특례 — 시행령 제15조 제1항 단서조항 명시

기존 부분에 건축허가일 기준을 적용하려면, 기존 부분과 증축 부분이 내화구조로 된 바닥·벽과 방화문(자동방화셔터 포함) 으로 구획된 경우에만 가능. 방화구획 미형성 시 기존 부분도 증축 허가일 기준 전면 적용. 체크박스로 방화구획 여부를 선택하면 결과 패널이 자동 분기.

Data
JS 
App
JS 
노유자시설은 노유자 이용시설과 생활시설로 분류해줘

오전 9:54

노유자시설을 생활시설과 이용시설로 분류 중
노유자시설을 생활시설과 이용시설로 분류 중
노유자시설의 생활시설·이용시설 구분은 화재안전기술기준(NFTC 103B 간이스프링클러)상 핵심 기준입니다. 생활시설은 24시간 거주형으로 면적 무관 설치 의무, 이용시설은 면적 기준이 적용됩니다.


명령 5개 실행함, 파일 읽음
명령 5개 실행함, 파일 읽음
data.js 수정 완료입니다. 분류 근거와 소방시설 적용기준 차이를 정리합니다.

노유자시설 생활시설·이용시설 구분 근거
분류 기준 (별표2 제9호 + NFTC 103B)
구분	ID	해당 시설	화재위험 특성
생활시설	welfare_living	양로시설·노인요양시설·노인요양공동생활가정·장애인거주시설·아동양육시설·정신요양시설·노숙인요양·재활시설·결핵·한센인 요양시설	24시간 거주·자력 피난 곤란자 상시 재실
이용시설	welfare_use	경로당·노인교실·어린이집·유치원·주야간보호·재가복지시설·장애인 지역사회재활·직업재활·노숙인일시보호·노인보호전문기관	주간 이용, 직원 상주·이동 가능
소방시설 적용기준 차이 (NFTC 103B 기준)
생활시설 (welfare_living) — 연면적·층수 무관 간이스프링클러 설치 의무

자력 피난이 사실상 불가능한 입소자가 상시 거주하므로, 화재 초기 진압이 곧 인명안전과 직결됨. 이는 사고 대응형 입법의 대표적 사례로 2014년 장성 효실천 요양원 화재(21명 사망) 이후 강화된 기준.

이용시설 (welfare_use) — 연면적 300㎡ 이상 간이스프링클러

이 구분은 본인의 논문 핵심 논지인 "사고 대응형 입법에 따른 소급 적용 문제"를 설명하는 실증 사례로도 활용하실 수 있습니다.
...
export const BUILDING_TYPES = [
  ...
  { id: "multiuse",         name: "[다중이용업소법 제9조] 다중이용업소 (별도 화재안전기준 적용)" },
];
...
export function getRequirements(type, area, gf, bf, occ, eraId) {
  ...
  if (!["apt","villa","warehouse","multiuse","geunlin"].includes(type) && sprMap[type]!==undefined) {
  ...
  if (["multiuse","underground"].includes(type))
    reqs.push({ cat: "소화설비", name: "간이스프링클러설비",
      cond: "전 장소 의무 (면적 무관)",
      std: std("103B"),
      art: type==="multiuse" ? "다중이용업소법 제9조 / " + std("103B") : std("103B") });
  ...
  const atdMap = { geunlin:600, dormitory:400, culture:400, religion:400, retail:500,
    ...
    underground_duct:0, heritage:300, mixed_a:400, mixed_b:400, multiuse:0 };
  ...
  if (["geunlin","apt","villa","dormitory","retail","welfare_living","welfare_use","lodging","multiuse"].includes(type) && !era0)
