...
export const BUILDING_TYPES = [
  ...
  { id: "welfare",          name: "[별표2 제9호] 노유자시설 (노인·아동·장애인·정신질환자·노숙인 관련 시설)" },
...
export function getRequirements(type, area, gf, bf, occ, eraId) {
  ...
  if (["geunlin","retail","entertainment","welfare"].includes(type) && area >= 200 && !era0)
  ...
  const sprMap = { medical:2000, retail:3000, welfare:600, lodging:600, entertainment:1000,
  ...
  if (type==="welfare" && !era0 && area>=300)
    reqs.push({ cat: "소화설비", name: "간이스프링클러설비 — 노유자시설",
      cond: "노유자시설 300㎡ 이상 [입력: " + area.toLocaleString() + "㎡]",
      std: std("103B"), art: std("103B") });
  ...
  const atdMap = { geunlin:600, dormitory:400, culture:400, religion:400, retail:500,
    transport:500, medical:300, education:400, welfare:300, training:500, sports:500,
  ...
  if (["welfare","medical","correctional"].includes(type) && area>=500 && !era0)
  ...
  if (["geunlin","apt","villa","dormitory","retail","welfare","lodging","multiuse"].includes(type) && !era0)
